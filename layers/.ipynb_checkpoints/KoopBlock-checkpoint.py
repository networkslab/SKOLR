...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.shareEncoder:
            if self.CI:
                self.encoder = MLP(f_in=self.seg_len, f_out=self.dynamic_dim, activation='relu', hidden_dim=self.hidden_dim, hidden_layers=self.hidden_layers, dropout=self.dropout)
                self.decoder = MLP(f_in=self.dynamic_dim, f_out=self.seg_len, activation='relu', hidden_dim=self.hidden_dim, hidden_layers=self.hidden_layers, dropout=self.dropout) 
            else:
                self.encoder = MLP(f_in=self.seg_len * self.enc_in, f_out=self.dynamic_dim, activation='relu', hidden_dim=self.hidden_dim, hidden_layers=self.hidden_layers, dropout=self.dropout) 
                self.decoder = MLP(f_in=self.dynamic_dim, f_out=self.seg_len * self.enc_in, activation='relu', hidden_dim=self.hidden_dim, hidden_layers=self.hidden_layers, dropout=self.dropout) 

            self.blocks = nn.ModuleList([TimeVarKP(self.enc_in, self.input_len, self.pred_len, self.seg_len, self.dynamic_dim, self.encoder, self.decoder, self.CI, self.inv_loss)for _ in range(self.num_blocks)
            ])
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # add this for branch analysis, and add related cin exp_main.py line 479
        block_pred = torch.stack(x_pred_list, dim=0) #[n, batch, seq_len, channels]
        block_pred = block_pred*std_enc + mean_enc/2
        return pred, block_pred
