...
class TimeVarKP(nn.Module):
    ...
    def forward(self, x):
        ...
        res = torch.stack(res, dim=1).reshape(B, self.freq, -1)   # B F PC [32, 2, 48 * 7]

        res = self.encoder(res) # B F H [32, 2, 64]
        x_rec, x_pred = self.dynamics(res, self.step) # B F H, B S H [32, 2, 64]

        x_rec = self.decoder(x_rec) # B F PC [32, 2, 48*7]
        x_rec = x_rec.reshape(B, self.freq, self.seg_len, self.enc_in) # [32, 2, 48, 7]
        x_rec = x_rec.reshape(B, -1, self.enc_in)[:, :self.input_len, :]  # B L C [32, 2*48, 7]
        
        x_pred = self.decoder(x_pred)     # B S PC [32, 1, 336(48*7)]
        x_pred = x_pred.reshape(B, self.step, self.seg_len, self.enc_in) #[32, 1, 48, 7]
        x_pred = x_pred.reshape(B, -1, self.enc_in)[:, :self.pred_len, :] # B S C [32, 48, 7]
